...
**************************************************************************/
float Balance_Pwm,Position_Pwm; //目标角度PWM、目标位置PWM
...
float D_Position_Current; //摆杆角度变化率
float D_Angle_Balance; //摆杆角度变化率
...
float Last_Angle_Balance; //用于获取摆杆角度变化率函数中，保存上一次角度
	...
	float x2 = 0.0;
	float x3 = 0.0;
	float Position_Current = 0.0;
	float Radian_Current = 0.0;
	float Position_0 = 0.0;
...
int TIM1_UP_IRQHandler(void)  
{
	...
	{   
		...
		state = (RuntimeState *)Framework_DataGet("runtime_state", 0);
		if (state == 0) {
			Set_Pwm(0);
			return 0;
		}
		Voltage = Get_battery_volt();           //===获取电池电压	     
		Angle_Balance = Get_Adc_Average(3,1);
		...
		Framework_TickFromISR(0.005f);  //===框架时间基准（5ms）
		Framework_RunOnce();
...
{
	u8 temp = 0; 
	if (0 != ErrorCode) { //电池电压过低，关闭电机
    temp=1;                                            
		BIN1=0;                                            
		BIN2=0;
  }
				
	if (!(Angle_Balance>(ZHONGZHI-500)&&Angle_Balance<(ZHONGZHI+500))) {
		ErrorCode=1;
		temp=1;
	}
