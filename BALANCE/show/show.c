...
#include "show.h"
#include "control.h"
#include "../../USER/core_framework.h"
#include "../../USER/runtime_state.h"
  /**************************************************************************
作者：平衡小车之家
...
void oled_show(void)
{
		...
		snprintf(str, sizeof(str), "%d:%4.2f", Menu==1?8:0, k0);
		OLED_ShowString(00,00,str);
		//memset(str, 0, sizeof(str));
		snprintf(str, sizeof(str), "%d:%4.2f", Menu==2?8:1, k1);
		OLED_ShowString(60,00,str);
		//memset(str, 0, sizeof(str));
		snprintf(str, sizeof(str), "%d:%4.2f", Menu==3?8:2, k2);
		OLED_ShowString(00,10,str);
		//memset(str, 0, sizeof(str));
		snprintf(str, sizeof(str), "%d:%4.2f", Menu==4?8:3, k3);
		OLED_ShowString(60,10,str);

		OLED_ShowNumber(00,20,ErrorCode,1,12);
	
		OLED_ShowNumber(40,20,state->is_initialized,1,12);
...
/*			
		...
		snprintf(str, sizeof(str), "PZ:%.3f", Position_0);
		OLED_ShowString(00,30,str);
		snprintf(str, sizeof(str), "PC:%.3f", Position_Current);
		OLED_ShowString(70,30,str);
			
		snprintf(str, sizeof(str), "PP:%.3f", Position_Pwm);
		OLED_ShowString(00,40,str);
		snprintf(str, sizeof(str), "BP:%.3f", Balance_Pwm);
		OLED_ShowString(70,40,str);
			
			
		// dtostrf(Position_Pwm, 0, 4, str);
		// OLED_ShowString(00,40,"B:");
...
{   
    ...
    DataScope_Get_Channel_Data( state->x[1] * 100.0f, 2 );
    DataScope_Get_Channel_Data( state->x[2] * 180.0f / 3.1415926, 3 );
    DataScope_Get_Channel_Data( state->x[3] * 180.0f / 3.1415926, 4 );
    DataScope_Get_Channel_Data( state->s * 100.0f, 5 );
    DataScope_Get_Channel_Data( state->x_d[2] * 180.0f / 3.1415926, 6 );
    DataScope_Get_Channel_Data( state->u_eq * 100.0f, 7 );
    DataScope_Get_Channel_Data( state->u_sw * 100.0f, 8 );
