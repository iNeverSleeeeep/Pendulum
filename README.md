...
### 控制器（LQR）
- LQR控制目前负责全状态反馈，同时稳定位置和角度，根据MATLAB仿真设置的增益，目前稳定性和位置跟踪性能较好。

### 控制器（外环MPC+内环SMC）
- MPC和SMC目前是配合使用，MPC负责位置环输出目标角度给SMC做内环角度控制，SMC有效控制了抖动，跟踪性能一般（角度跟踪上升时间0.25s）。
...
- 目前存在的问题 1. L1是对电压进行补偿，在我的导轨长度比较短+磁铁比较重的情况，无法进行验证，必须要对目标角度进行补偿才可以。
